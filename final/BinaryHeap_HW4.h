...
private:
    ...
    void printHeap(int, int) const; // printHeap() is a private accessor member function and is tagged as const

public: 

    BinaryHeap(): heap(100), currentSize(0) {} // By default vector<C> heap is declared with an initial size of 100 and int curentSize is initialized to 0

    BinaryHeap(int capacity): heap(capacity), currentSize(0) {} // private data field, int currentSize can be set when BinaryHeap objects is instantiated
  
    BinaryHeap(const vector<C> &); // BinaryHeap object upon instantiation, can be passed a vector to be contained in private data field vector<C> heap
